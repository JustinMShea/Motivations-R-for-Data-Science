...
Josh has: **Substantive expertise, Math & Statistics, and Data Hacking Skills:**
...
Results indicate `Python` the most popular, with `R` 2nd, and `SAS` 3rd and losing market share rapidly. Reflects a general shift to programming languages away from commercial analytics products.

**NOTE** Burchworks doesn't appear to be using best practices in Data Science, Reproducible Research, or Survey methods, creating much ire among data professionals every year! 

So don't quote this as gospel!


[Source: Full '2019 Burtchworks Survey' HERE](https://www.burtchworks.com/2019/08/21/2019-sas-r-or-python-survey-update-which-tool-do-data-scientists-analytics-pros-prefer/)

```{r, out.width = "500px"}
path <- paste0(getwd(), "/images/burtch-Preference-2019.png")
knitr::include_graphics(path)
```

## IEEE, 2018 Top Programming languages {.columns-2 .smaller}


Amazingly, in the wider world of programming languages, the `R Statistical Programming language`,
created by Data Scientists for **Data Scientists**, makes the "top 10" general programming language cut under the **Enterprise Catagory**, reflecting it's growing use across scientific domains, business sectors, and Enterprises. It is very surprising for a specialized language to rank so high.


`Python`'s popularity is due to it's high use in Web site development, DevOps, and as a general programming language. Use in web development and recent adaptation in **Data Science** has put it on top.
...
```{r, out.width = "500px"}
path <- paste0(getwd(), "/images/ieee-10-2018.PNG")
knitr::include_graphics(path)
```

[Source: IEEE 2018 top programming languages](https://spectrum.ieee.org/static/interactive-the-top-programming-languages-2018)
[Source: IEEE 2018 Design, Methods, and Data](https://spectrum.ieee.org/static/ieee-top-programming-languages-2018-methods)


## Stackoverflow 2017 analysis of Languages {.smaller}
...
# Code from https://blog.revolutionanalytics.com/2015/06/how-many-packages-are-there-really-on-cran.html
...
pkgNumber <- NROW(pkgs$source)
```

Years ago, I discovered `R` because I needed to use the most cutting edge, statistical modeling libraries available. With over `r pkgNumber` packages available beyond base `R` as of this rendering, the community organizes noteworthy and mature approaches by research domain. They can ALL be installed with a few commands.
...
Bridges statistics and computer science. Many early innovators among `S` & `R` communities at Stanford, Berkeley, and Bell Labs.
...
_The essence of what we took from him (Tukey) in thinking about S, for example, can be summarized in another of his famous one-liners, which goes roughly like this:_
...
## An R Projects workflow {.smaller .columns-2}


**First Mile**: _Import_ data and prepare for Analysis. Clean and format data. For example, make each variable a column, each observation is a row, etc. Alot of time is spent here.

**Data Analysis**: _Transform_ data and create new variables with `log`, `exponents`, differencing, etc. _Visualizations_ help spot outliers, remaining data problems, and reveal insights to test with _Models_. Predictive modeling frameworks like the [mlr (Machine Learning in R) package ](https://mlr.mlr-org.com/) provide tools to quickly iterate over the entire process.
...
path <- paste0(getwd(), "/images/Practical-Data-Science-project-workflow.jpg")
knitr::include_graphics(path)
```

[Image Source: Practical Data Science with R](https://www.manning.com/books/practical-data-science-with-r-second-edition)
...
- Continually study Statistics and Machine Learning, that is core of the work.

- [Practical Data Science with R](https://www.manning.com/books/practical-data-science-with-r-second-edition) is an excellent book

- [Datacamp.com](https://www.datacamp.com/) is excellent for learning how to do specific things and learn the language of R and Python.

- Signup for R, Python, and general Data [meetups](https://www.meetup.com/) and go! They are free, keep you current.
